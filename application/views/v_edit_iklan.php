...
<body>
  ...
  <div class="modal fade" id="staticModalEdit" tabindex="-1" role="dialog" aria-labelledby="staticModalLabel" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-sm" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticModalLabel">Edit Iklan</h5>
        ...
        <div class="modal-body">
          <p>
            Apakah anda yakin mengedit iklan ini?
          </p>
        </div>
        <div class="modal-footer">
          <a href="javascript:void(0);" class="btn btn-secondary" data-dismiss="modal">Cancel</a>
          <a href="javascript:void(0);" class="btn btn-warning" onclick="update_data_iklan()">Confirm</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <section class="checkout_area p_120">
      ...
      <div class="billing_details ">
        ...
        <div class="col-lg-8 offset-lg-2">
          ...
          <div class="text-center mb-3">
            <form class="row contact_form" novalidate="novalidate" id="form_edit_profil" method="post" enctype="multipart/form-data">
              ...
              <div class="col-md-12 form-group">
                ...
                <img id="foto_iklan" alt="aaaa" style="width: 20%">
                <input type="file" class="form-control" id="foto_ikl" name="filefoto">
              </div>

              <div class="col-md-12 form-group">
                <a href="javascript:void(0);" class="main_btn" id="btn_edit_iklan" data-toggle="modal" data-target="#staticModalEdit">Simpan</a>
              </div>
    ...
    function edit_iklan(){
      $.ajax({
        type: "post",
        url:"<?php echo base_url('index.php/c_iklan/edit/'); ?>"+id_ikl,
        dataType: "json",
        success: function(data){
          console.log(data);
          //document.getElementById("profil_pic").src = 'http://localhost/pro/assets/uploads/'+data[0].FOTO_USER;
          $('[name="judul"]').val(data[0].JUDUL_IKLAN);
          if(data[0].KATEGORI == 'Elektronik'){
            document.getElementById("select_jns_k").selectedIndex = 1;
          }else if(data[0].KATEGORI == 'Komputer'){
            document.getElementById("select_jns_k").selectedIndex = 2;
          }else if(data[0].KATEGORI == 'Otomotif'){
            document.getElementById("select_jns_k").selectedIndex = 3;
          }else if(data[0].KATEGORI == 'Fashion'){
            document.getElementById("select_jns_k").selectedIndex = 4;
          }else{
            document.getElementById("select_jns_k").selectedIndex = 5;
          }
          $('[name="harga"]').val(data[0].HARGA);
          $('[name="deskripsi"]').val(data[0].DESKRIPSI);
          document.getElementById("foto_iklan").src = 'http://localhost/pro/assets/uploads/'+data[0].FOTO_IKLAN;
        }
      });
    }

    function update_data_iklan(){
      var data_upd_iklan = new FormData();

      data_upd_iklan.append('id_iklan',id_ikl);
      data_upd_iklan.append('judul',$('[name="judul"]').val());
      data_upd_iklan.append('kategori',$('[name="kategori"]').val());
      data_upd_iklan.append('harga',$('[name="harga"]').val());
      data_upd_iklan.append('deskripsi',$('[name="deskripsi"]').val());
      data_upd_iklan.append('foto',$("#foto_ikl")[0].files[0]);
      console.log($('[name="judul"]').val());
      $.ajax({
        url:"<?php echo base_url('index.php/c_iklan/update')?>",
        type: "post",
        data:  data_upd_iklan,
        processData: false,
        contentType: false,
