...
<body>
  ...
  <div class="modal fade" id="staticModalEditProfil" tabindex="-1" role="dialog" aria-labelledby="staticModalLabel" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-sm" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticModalLabel">Edit Profil</h5>
        ...
        <div class="modal-footer">
          ...
          <a href="javascript:void(0);" class="btn btn-warning" onclick="update_profil()">Confirm</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <section class="checkout_area p_120">
        ...
        <div class="col-lg-8 offset-lg-2">
          ...
          <div class="text-center mb-3">
            <div class="img">
              ...
              <img class="gambar author_img rounded-circle" id="profil_pic" alt="aaaa" style="width: 20%">
              <input type="file" name="file" id="foto_profil" class="inputfile" />



            </div>
            <form class="row contact_form" novalidate="novalidate" id="form_edit_profil">

              <div class="col-md-12 form-group">
                <input type="text" class="form-control" id="nama" name="nama">
              </div>
              <div class="col-md-6 form-group p_star">
                <input type="text" class="form-control" id="telpno" name="telpno" placeholder="Nomor Telepon">
              </div>
              <div class="col-md-6 form-group">
                <input type="email" class="form-control" id="email" name="email" placeholder="Email">
              </div>
              <div class="col-md-6 form-group p_star">
                <input type="text" class="form-control" id="birth_addr" name="birth_addr" placeholder="Tempat Lahir">
              </div>
              <div class="col-md-6 form-group p_star">
                <input type="date" class="form-control" id="birth_date" name="birth_date" placeholder="Tanggal Lahir">
              </div>
              <div class="col-md-12 form-group">
                <input type="text" class="form-control" id="alamat" name="alamat" placeholder="Alamat">
              </div>
              <div class="col-md-12 form-group p_star">
                <select class="country_select form-control" id="select_jns_k" name="jenis_kelamin">
              ...
              <div class="col-md-12 form-group">
                <a href="javascript:void(0);" class="main_btn" id="btn_update_profil" data-toggle="modal" data-target="#staticModalEditProfil">Simpan</a>
              </div>
              <p id="demo"></p>
    ...
    function profil(){
      $.ajax({
        url:'<?=base_url()?>index.php/c_profil/data_profil',
        type: 'post',
        dataType: 'json',
        success: function(data){
          console.log(data);
          document.getElementById("profil_pic").src = 'http://localhost/pro/assets/uploads/'+data[0].FOTO_USER;
          $('[name="nama"]').val(data[0].NAMA_USER);
          $('[name="telpno"]').val(data[0].NO_HP);
          $('[name="email"]').val(data[0].EMAIL);
          $('[name="birth_addr"]').val(data[0].TEMPAT_LAHIR);
          $('[name="birth_date"]').val(data[0].TANGGAL_LAHIR);
          if(data[0].JENIS_KELAMIN == 'L'){
            document.getElementById("select_jns_k").selectedIndex = 1;
          }else{
            document.getElementById("select_jns_k").selectedIndex = 2;
          }
    ...
    function update_profil(){
      var data_upd_profil = new FormData();

      data_upd_profil.append('id_user',"<?php echo $this->session->userdata('id');?>");
      data_upd_profil.append('nama_user',$('[name="nama"]').val());
      data_upd_profil.append('telpno',$('[name="telpno"]').val());
      data_upd_profil.append('email',$('[name="email"]').val());
      data_upd_profil.append('birth_addr',$('[name="birth_addr"]').val());
      data_upd_profil.append('birth_date',$('[name="birth_date"]').val());
      data_upd_profil.append('alamat',$('[name="alamat"]').val());
      data_upd_profil.append('jenis_kelamin',$('[name="jenis_kelamin"]').val());
      data_upd_profil.append('username',$('[name="username"]').val());
      data_upd_profil.append('password',$('[name="password"]').val());
      data_upd_profil.append('foto',$("#foto_profil")[0].files[0]);

      console.log(<?php echo $this->session->userdata('id');?>);
      $.ajax({
        url:"<?php echo base_url('index.php/c_user/update_data_profil')?>",
        type: "post",
        data: data_upd_profil,
        processData: false,
        contentType: false,
        success: function(data){
          console.log(data);
          $('#staticModalEditProfil').modal('hide');
          profil();
        }
      });
      return false;
